...
import { FolderKanban, MessageSquare, FileText, AlertCircle, CheckCircle2, Clock, ChevronRight, Sparkles } from 'lucide-react'
    ...
    <div style={{ border: '1px solid rgba(255,255,255,0.07)', borderRadius: 16, padding: '20px 24px', background: 'rgba(255,255,255,0.02)', display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
          {newMessages > 0 && (
            <Link href={`/client/projets/${project.id}/messages`} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '3px 8px', borderRadius: 999, background: 'rgba(232,148,106,0.12)', border: '1px solid rgba(232,148,106,0.25)', color: '#e8946a', fontSize: 11, textDecoration: 'none' }}>
      ...
      {total > 0 && (
        <div>
          ...
          <div style={{ fontSize: 11, color: 'rgba(240,235,228,0.3)', marginTop: 4 }}>{done}/{total} tâches complétées</div>
        </div>
      )}
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        ...
        <Link href={`/client/projets/${project.id}/documents`} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '5px 12px', borderRadius: 6, border: '1px solid rgba(255,255,255,0.08)', color: 'rgba(240,235,228,0.55)', fontSize: 12, textDecoration: 'none' }}>
          <FileText size={11} strokeWidth={1.8} /> Documents
        </Link>
...
export default function ClientDashboard() {
  ...
  const { projects, recentMessages, recentRequests, recentDocs } = data
